...
const AdminProjects = () => {
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-display text-slate-900">{editing ? "Edit Project" : "New Project"}</h1>
        ...
        <div className="max-w-2xl space-y-6">
          ...
          <div>
            ...
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={6} className="w-full px-4 py-3 bg-slate-100 border border-slate-200 rounded-md text-slate-900/90 focus:outline-none focus:border-[hsl(var(--accent))] resize-none" />
          </div>
  ...
  return (
    <div>
      ...
      : items.length === 0 ? <div className="text-center py-20 text-slate-400">No projects yet.</div>
      : (
          ...
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-4 bg-white/[0.03] border border-slate-200 rounded-lg p-4 hover:border-slate-200 transition-colors">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-slate-500 text-sm">{item.client} · {item.category} · {item.status}</p>
              </div>
...
const Select = ({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (v: string) => void }) => (
  <div><label className="block text-slate-600 text-sm mb-2">{label}</label><select value={value} onChange={(e) => onChange(e.target.value)} className="w-full px-4 py-3 bg-slate-100 border border-slate-200 rounded-md text-slate-900/90 focus:outline-none focus:border-[hsl(var(--accent))]">{options.map(o => <option key={o} value={o}>{o}</option>)}</select></div>
);
