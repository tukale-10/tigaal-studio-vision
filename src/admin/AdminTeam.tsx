...
const AdminTeam = () => {
    ...
    return (
      <div>
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-2xl font-display text-white">{editing ? "Edit Member" : "New Member"}</h1>
        ...
        <div className="max-w-2xl space-y-6">
          ...
          <div>
            ...
            <textarea value={form.bio} onChange={(e) => setForm({ ...form, bio: e.target.value })} rows={5} className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-md text-white/90 focus:outline-none focus:border-[hsl(var(--accent))] resize-none" />
          </div>
          <div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-display text-white">Team Members</h1>
      ...
      : items.length === 0 ? <div className="text-center py-20 text-white/30">No team members yet.</div>
      : (
        <div className="space-y-2">
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-4 bg-white/[0.03] border border-white/5 rounded-lg p-4 hover:border-white/10 transition-colors">
              {item.image_url ? <img src={item.image_url} alt="" className="w-12 h-12 object-cover rounded-full flex-shrink-0" /> : <div className="w-12 h-12 bg-white/10 rounded-full flex-shrink-0" />}
              <div className="flex-1 min-w-0">
                <h3 className="text-white font-medium truncate">{item.name}</h3>
                <p className="text-white/40 text-sm truncate">{item.title}</p>
              </div>
